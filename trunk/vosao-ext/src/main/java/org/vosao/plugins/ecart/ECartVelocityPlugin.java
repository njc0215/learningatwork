...
package org.vosao.plugins.ecart;
...
public class ECartVelocityPlugin extends AbstractVelocityPlugin {

	private static final Log logger = LogFactory.getLog(
			ECartVelocityPlugin.class);

	public ECartVelocityPlugin(Business business) {
		setBusiness(business);
	}
	
	private String renderSitemap(String templateUrl) {
		try {
			PluginEntity plugin = getDao().getPluginDao().getByName("ecart");
			ECartConfig config = getConfig(plugin);
			...
			String template = StreamUtil.getTextResource(
				ECartVelocityPlugin.class.getClassLoader(), 
				templateUrl);
	...
	public String render() {
		return renderSitemap("org/vosao/plugins/ecart/ecart.vm");
	}
	...
	private ECartConfig getConfig(PluginEntity plugin) {
		Map<String, PluginParameter> params = PluginHelper.parseParameters(
				plugin);
		ECartConfig result = new ECartConfig();
		try {
	...
	public String renderXML() {
		return renderSitemap("org/vosao/plugins/ecart/ecartxml.vm");
	}
