...
public class RtspServer extends JFrame implements ActionListener {
	...
	private static final int RTSPport = 1234;
	
	private static Logger log = LoggerFactory.getLogger(RtspServer.class);
...
//	final static int TEARDOWN = 6;
	...
	public RtspServer() {
		...
		// init Frame
		super("RTSP Server - listening port:"+RTSPport);
	...
	public static void main(String argv[]) throws Exception {
		...
		log.info("The RTSP Server is running at port "+RTSPport);
		...
		// Initiate RTSPstate
		theServer.updateState(RtspState.INIT);
		...
		while (!done) {
			...
			if (request_type == RtspRequest.SETUP) {
				...
				// update RTSP state
				theServer.updateState(RtspState.READY);
		...
		while (true) {
			// parse the request
			log.info("wait for RTSP request ... ");
			request_type = theServer.parse_RTSP_request(); // blocking
			
			log.info("RTSP request: "+request_type);
			...
			if ((request_type == RtspRequest.PLAY) && (state == RtspState.READY)) {
				...
				// update state
				theServer.updateState(RtspState.PLAYING);
	...
	private void updateState(RtspState newState){
		this.state = newState;
...
//		log.info("event : "+e.getSource());
		if (imagenb < VIDEO_LENGTH) {
			...
			try {
//				log.info("get next frame to send from the video ...");
...
//				rtp_packet.printheader();
	...
	// Parse RTSP Request
	// ------------------------------------
	private RtspRequest parse_RTSP_request() {
		RtspRequest request_type = RtspRequest.UNKNOWN;
		try {
			// parse request line and extract the request_type:
			String RequestLine = RTSPBufferedReader.readLine();
			log.info("====== parse RTSP request ====");
			...
			if (request_type == RtspRequest.SETUP) {
				...
				log.info(">"+LastLine);
				log.info("extract RTP_dest_port from LastLine ...");
				tokens = new StringTokenizer(LastLine);
				for (int i = 0; i < 3; i++)
					tokens.nextToken(); // skip unused stuff
				RTP_dest_port = Integer.parseInt(tokens.nextToken());
				log.info("RTP_dest_port = "+RTP_dest_port);
			}
			else
			{
		...
		log.info("Now return the request_type:"+request_type);
		return (request_type);
	}
	...
	private void send_RTSP_response() {
		try {
			...
			log.info("RTSP Server - Sent response to Client.");
		} catch (Exception ex) {
			log.warn("Exception caught: " + ex);
			System.exit(0);
		}
